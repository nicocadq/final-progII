...
CREATE TABLE Exam( idExam INT NOT NULL AUTO_INCREMENT, ciStudent INT NOT NULL, idSubject VARCHAR(64), date DATE, PRIMARY KEY (idExam, ciStudent), FOREIGN KEY (idSubject) REFERENCES Subject(idSubject), FOREIGN KEY (ciStudent) REFERENCES Student(ciUser));
...
CREATE TABLE Took(idExam INT NOT NULL, ciStudent INT NOT NULL, markFailed INT, PRIMARY KEY(idExam, ciStudent), FOREIGN KEY (idExam) REFERENCES Exam(idExam), FOREIGN KEY (ciStudent) REFERENCES Student(ciUser));
